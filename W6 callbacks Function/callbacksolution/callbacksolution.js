...
function filterArray(myArray, isEvenCallBack) {
  ...
  //for (let i = 0; i < myArray.length; i++) {    if (isEvenCallBack(myArray[i])) evenArray.push(myArray[i]);  }
  myArray.forEach(element => {
    if (isEvenCallBack(element)) evenArray.push(element);
  });
  return evenArray;
}
...
if (typeof module !== "undefined") {  module.exports = { filterArray, isEvenCallBack };}
